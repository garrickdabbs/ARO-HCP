...
func TestLoadCatalogAndExpandSlots(t *testing.T) {
	...
	if len(devSlots) != 15 {
		t.Fatalf("expected 15 dev slots, got %d", len(devSlots))
	}

	firstDevSlot := devSlots[0]
	expectedSubscriptionName := catalog.Environments["dev"].Pools[0].SubscriptionName
	if firstDevSlot.ResourceType != "aro-hcp-dev-westus3-slot" {
		t.Fatalf("unexpected resource type %q", firstDevSlot.ResourceType)
	}
	if firstDevSlot.ResourceName != "aro-hcp-dev-westus3-slot-00" {
	...
	if firstDevSlot.IdentityContainerPrefix != "aro-hcp-msi-container-dev-00" {
	...
	if identityContainers[0] != "aro-hcp-msi-container-dev-00-00" {
		t.Fatalf("unexpected first identity container %q", identityContainers[0])
	}
	if identityContainers[19] != "aro-hcp-msi-container-dev-00-19" {
...
func TestResolvePool(t *testing.T) {
	...
	pool, err := catalog.ResolvePool("dev", catalog.Environments["dev"].Pools[0].SubscriptionName, "westus3")
	if err != nil {
		t.Fatalf("expected pool resolution to succeed: %v", err)
	}
	if pool.ResourceType != "aro-hcp-dev-westus3-slot" {
...
func TestFindSlotByResourceName(t *testing.T) {
	...
	slot, err := catalog.FindSlotByResourceName("aro-hcp-prod-uksouth-slot-09")
	if err != nil {
	...
	if slot.IdentityContainerPrefix != "aro-hcp-msi-container-prod-09" {
