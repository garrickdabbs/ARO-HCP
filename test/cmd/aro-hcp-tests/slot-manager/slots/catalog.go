...
type Pool struct {
	...
	SubscriptionIDHash      string `yaml:"subscription_id_hash"`
...
func (c *Catalog) Validate() error {
	...
	for _, environmentName := range c.EnvironmentNames() {
		...
		for _, pool := range environment.Pools {
			...
			case strings.TrimSpace(pool.SubscriptionIDHash) == "":
				return fmt.Errorf("environment %q has a pool with empty subscription_id_hash", environmentName)
...
func ExpandSlotsForPool(environment string, pool Pool) []ExpandedSlot {
	...
	for i := 0; i < pool.SlotCount; i++ {
		slots = append(slots, ExpandedSlot{
			...
			SubscriptionIDHash:      pool.SubscriptionIDHash,
			Region:                  pool.Region,
