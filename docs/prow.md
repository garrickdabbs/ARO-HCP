...
- **`slot-manager` CLI**
  ...
  - It wraps a pre‑generated ARM template (`msi-pools.json`, built from `test/e2e-setup/bicep/msi-pools.bicep`) and applies it as Azure deployment stacks, one per slot:
    ...
    - It loads the canonical slot catalog, expands the slots for the requested environment/pool, and creates container RGs from each slot's prefix and count.
    - It validates the current `CUSTOMER_SUBSCRIPTION` against the selected pool's `subscription_name` before applying anything.
  - Example usage (run from the `test` image or a local build):
    ...
    - `./test/aro-hcp-tests slot-manager apply-identity-pool --environment prod --subscription-name prod --region uksouth`
